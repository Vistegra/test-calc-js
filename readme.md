...
Расстояние между лагами заполнить равномерно, с учетом максимального расстояния в метрах (поле step, тип frame из конфига) и учитывая ширину трубы в мм (поле "width" из БД). Ячейка не обязательно должна быть квадратной.
